 ...
 * 
 * Rules based on
 * http://code.zikula.org/phpcs/browser/trunk/Eclipse/PHPFormatter
 * /ZFormat.xml?format=txt (named reference below)
 */
public class PreferenceInitializer extends AbstractPreferenceInitializer {
    ...
    public void initializeDefaultPreferences() {
        ...
        // Join '.' to previous line (only none or one of previous or post)
        // (reference: none)
        store.setDefault(PreferenceConstants.JOIN_CONCAT_OP_TO_PREV_LINE, false);
        // Join '.' to post line (only none or one of previous or post)
        // (reference: none)
        ...
        store.setDefault(PreferenceConstants.LEAVE_NEWLINE_FOR_ARROW_NEST,
                false);
        // Equate 'else if' to 'elseif' (reference: none)
        store.setDefault(PreferenceConstants.EQUATE_ELSE_IF_TO_ELSEIF, true);
        // Simple statement in one line (reference:
        // keep_imple_if_on_one_line=false)
        store.setDefault(PreferenceConstants.SIMPLE_STATEMENT_IN_ONE_LINE,
                false);
        // No new line in empty block ('{}') (reference:
        ...
        // Insert spacer for => operator in array (reference:
        // insert_space_after_arrow_in_array_creation=true)
        store.setDefault(PreferenceConstants.SPACER_FOR_ARRAY_ARROW, true);
        // Insert spacer for function definition (reference:
        ...
        // Insert spacer for casting (reference:
        // insert_space_after_closing_paren_in_cast=false)
        store.setDefault(PreferenceConstants.SPACER_FOR_CAST, false);

        // Insert spacer after if/for/foreach/while/switch/do
        ...
        // Insert spacer after '}' in else/elseif/catch
        store.setDefault(PreferenceConstants.SPACER_AFTER_CURLY, true);

        /** MISCELLANEOUS */
        // Indent case block (reference:
    ...
    public void copyPreference(IPreferenceStore master, IPreferenceStore slave) {
        ...
        copyBooleanValue(master, slave,
                PreferenceConstants.LEAVE_NEWLINE_WITH_CONCAT_OP);
        copyBooleanValue(master, slave,
                PreferenceConstants.LEAVE_NEWLINE_FOR_ARROW);
        copyBooleanValue(master, slave,
                PreferenceConstants.LEAVE_NEWLINE_FOR_ARROW_NEST);
        copyBooleanValue(master, slave,
                PreferenceConstants.JOIN_CONCAT_OP_TO_PREV_LINE);
        copyBooleanValue(master, slave,
                PreferenceConstants.JOIN_CONCAT_OP_TO_POST_LINE);
        copyBooleanValue(master, slave, PreferenceConstants.ALIGN_DOUBLE_ARROW);
        copyBooleanValue(master, slave,
                PreferenceConstants.ALIGN_DOUBLE_ARROW_WITH_TAB);
        copyBooleanValue(master, slave,
    ...
    private void copyBooleanValue(IPreferenceStore master,
            IPreferenceStore slave, String name) {
        slave.setValue(name, master.getBoolean(name));
    }

    private void copyIntValue(IPreferenceStore master, IPreferenceStore slave,
            String name) {
